#!/usr/bin/env bash
# 실험1 (per-category, mask_rcnn) — GPU 0 전용
# 6 카테고리 × 6 conditions = 36 runs, 매 run 후 10분 쿨다운
#
# 사용:
#   nohup bash scripts/run_exp1_singlecat_gpu0.sh > results/logs/exp1_master.log 2>&1 &
...
LOG_DIR="results/logs/exp1_singlecat_${TS}"
SUMMARY_MD="results/reports/exp1_singlecat_${TS}.md"
...
echo "================================================================"
echo "  실험1 (single-category) GPU 0  시작: $START_TS"
...
for cat in "${CATEGORIES[@]}"; do
  for cond in "${CONDITIONS[@]}"; do
    ...
    python scripts/summarize_exp1_singlecat.py --output "$SUMMARY_MD" || true
