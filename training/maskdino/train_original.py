#!/usr/bin/env python3
"""
MaskDINO Training Script - Original Data Only (Fixed for BitMask)
Trains on original images only (000000.jpg ~ 000039.jpg)
"""
...
class MaskDINODatasetMapper:
    ...
    def __init__(self, cfg, is_train=True):
        ...
        self.mask_format = cfg.INPUT.MASK_FORMAT  # Will be converted to bitmask anyway
...
# ============================================================
def filter_original_only(annotations_path, images_dir):
    """Filter to keep only original images (000000.jpg ~ 000039.jpg)"""
    with open(annotations_path, 'r') as f:
        coco_data = json.load(f)
    
    # Original image filenames
    original_files = set(f"{i:06d}.jpg" for i in range(40))
    
    # Filter images
    filtered_images = [img for img in coco_data['images'] if img['file_name'] in original_files]
    ...
    print(f"Original data: {len(filtered_images)} images, {len(filtered_annotations)} annotations")
...
def register_dataset(name, annotations_path, images_dir, filtered_data=None):
...
def setup_cfg():
    ...
    # Dataset settings
    cfg.DATASETS.TRAIN = ("thunderbolt_original_train",)
    cfg.DATASETS.TEST = ("thunderbolt_original_train",)  # Using train for eval too
    
    # Model settings
    cfg.MODEL.WEIGHTS = "detectron2://ImageNetPretrained/torchvision/R-50.pkl"
    cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES = 1
    
    # Input settings - Keep as polygon, our mapper will convert
    ...
    # Training settings
    cfg.SOLVER.IMS_PER_BATCH = 1  # Small batch for limited data
    ...
    # Output
    cfg.OUTPUT_DIR = "/data2/project/2026winter/jjh0709/AA_CV_R/output_original"
...
def main():
    ...
    filtered_data = filter_original_only(train_annotations, train_images)
    
    # Register datasets
    register_dataset(
        "thunderbolt_original_train",
        train_annotations,
        train_images,
        filtered_data
    )
    ...
    print("="*60)
    print("MaskDINO Training - Original Data Only")
    print("="*60)
